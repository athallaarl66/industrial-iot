...
export function DeleteDialog({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-6 animate-in fade-in duration-300">
      <div className="industrial-panel bg-white p-10 max-w-lg w-full shadow-2xl border-slate-200 transform animate-in slide-in-from-bottom-8 duration-500">
        ...
        <p className="text-sm font-bold text-slate-500 mb-10 leading-relaxed uppercase tracking-tight">
          {message}
        </p>
        ...
        <div className="flex justify-end space-x-4">
          <button
            onClick={onClose}
            disabled={isDeleting}
            className="px-6 py-3 text-xs font-black text-slate-400 uppercase tracking-[0.2em] hover:text-slate-600 transition-colors disabled:opacity-50"
          >
            Abort
          </button>
          <button
